...
from llm_provider.retry import RetryConfig, calculate_backoff_delay, retry_with_backoff
...
class TestRetry(unittest.TestCase):
    ...
    def test_retry_with_backoff_non_retryable_error(self):
        ...
        func = Mock(
            side_effect=ValidationError(
                error_type=ErrorType.INVALID_CONFIGURATION, message="Validation failed"
            )
        )
    ...
    def test_retryable_decorates_async_function(self):
        ...
        @retryable(
            operation_name="test_async", retry_config=RetryConfig(max_retries=1, base_delay=0.01)
        )
    ...
    def test_async_retryable_uses_asyncio_sleep(self):
        ...
        @retryable(
            operation_name="test_async_sleep",
