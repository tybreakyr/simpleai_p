...
from .models import FactoryConfig, Model, ProviderConfig, ProviderFeatures
...
class ProviderFactory:
    ...
    def __init__(self, config: FactoryConfig | None = None):
        ...
        self._config: FactoryConfig | None = None

        if config:
            self.load_config(config)

    def register_provider(
        self, name: str, constructor: Callable[[dict[str, Any]], Provider]
    ) -> None:
        """
    ...
    def create_provider(self, name: str, config: ProviderConfig) -> Provider:
        ...
        with self._lock:
            ...
            try:
                ...
                raise InvalidConfigurationError(
                    f"Failed to create provider '{name}': {str(e)}", cause=e
                ) from e
    ...
    def list_models(self, provider_name: str) -> list[Model]:
        ...
        with self._lock:
            ...
            try:
                ...
                raise InvalidConfigurationError(
                    f"Failed to list models for provider '{provider_name}': {str(e)}", cause=e
                ) from e
    ...
    def get_config(self) -> FactoryConfig | None:
        """
