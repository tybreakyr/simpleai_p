...
def validate_factory_config(config: FactoryConfig) -> None:
    ...
    if field_errors:
        raise ValidationError(
            message="Factory configuration validation failed", field_errors=field_errors
        )
...
def validate_provider_config(config: ProviderConfig) -> None:
    ...
    if field_errors:
        raise ValidationError(
            message="Provider configuration validation failed", field_errors=field_errors
        )
...
def load_factory_config_from_dict(config_dict: dict[str, Any]) -> FactoryConfig:
    ...
    try:
        ...
        raise InvalidConfigurationError(
            f"Failed to load factory configuration: {str(e)}", cause=e
        ) from e
...
def load_factory_config_from_json(json_str: str) -> FactoryConfig:
    ...
    try:
        ...
        raise InvalidConfigurationError(
            f"Failed to parse JSON configuration: {str(e)}", cause=e
        ) from e
