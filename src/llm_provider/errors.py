...
def classify_error(
    error_message: str, status_code: int | None = None, exception: Exception | None = None
    ...
    if any(
        term in error_message_lower for term in ["connection", "network", "unreachable", "refused"]
    ):
    ...
    if status_code in (401, 403) or any(
        term in error_message_lower for term in ["unauthorized", "forbidden", "auth", "api key"]
    ):
        return (ErrorType.OPERATION_FAILED, False)

    # Not found errors - not retryable
    if status_code == 404 or any(
        term in error_message_lower for term in ["not found", "does not exist"]
    ):
